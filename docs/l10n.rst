...
.. code-block:: jinja
    ...
    <html>
    <head>
        <title>{{ ftl('example_page_title') }}</title>
    </head>
    <body>
        <h1>{{ ftl('example_page_title') }}</h1>
        <p>{{ ftl('example_page_desc') }}</p>
        <footer>
            <p>{{ ftl('example_footer') }}</p>
        </footer>
    </body>
    </html>
...
2. Words should be separated with underscores.
3. IDs should be prefixed with the name of the template file (e.g. ``firefox_new_skyline`` for ``firefox-new-skyline.html``)
4. If you need to create a new string for the same place on a page and to transition to it as it is translated, you can
   add a version suffix to the string ID: e.g. ``firefox_new_skyline_main_page_title_v2``.
...
.. code-block:: python
    ...
    def about_view(request):
        if has_messages('about_hello_v2', 'about_title_v2'):
            hello_string = ftl('about_hello_v2')
            title_string = ftl('about_title_v2')
        else:
            hello_string = ftl('about_hello')
            title_string = ftl('about_title')
